...
const packagesService_1 = require("./packagesService");
class LabelService {
    /**
     * Generar etiqueta de paquete en PDF
     */
    static async generatePackageLabel(packageId, tenantId) {
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                // Generar QR code
                const qrCodeDataUrl = await qrcode_1.default.toDataURL(pkg.tracking_number, {
                    width: 150,
                    margin: 1
                });
     ...
     */
    static async generateDeliveryNote(packageId, tenantId) {
        const pkg = await packagesService_1.PackagesService.getById(packageId, tenantId);
        if (!pkg) {
            throw new Error('Paquete no encontrado');
        }
