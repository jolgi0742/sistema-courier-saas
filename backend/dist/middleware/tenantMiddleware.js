...
exports.superAdminMiddleware = exports.optionalTenantMiddleware = exports.tenantMiddleware = void 0;
...
const tenantMiddleware = async (req, res, next) => {
    try {
        ...
        tenant = await tenantService_1.TenantService.getByCustomDomain(hostname);
        // 2. Si no, intentar por subdominio
        if (!tenant) {
            const subdomain = hostname.split('.')[0];
            // Ignorar subdominios del sistema
            if (!['www', 'api', 'admin', 'app'].includes(subdomain)) {
                tenant = await tenantService_1.TenantService.getBySubdomain(subdomain);
            }
        }
        // 3. Verificar header X-Tenant-ID (para APIs)
        if (!tenant && req.headers['x-tenant-id']) {
            tenant = await tenantService_1.TenantService.getById(req.headers['x-tenant-id']);
        }
...
const optionalTenantMiddleware = async (req, res, next) => {
    try {
        ...
        let tenant = await tenantService_1.TenantService.getByCustomDomain(hostname);
        if (!tenant) {
            const subdomain = hostname.split('.')[0];
            if (!['www', 'api', 'admin', 'app'].includes(subdomain)) {
                tenant = await tenantService_1.TenantService.getBySubdomain(subdomain);
        ...
        if (tenant && tenant.status === 'active') {
            req.tenant = tenant;
            req.branding = await brandingService_1.BrandingService.getByTenantId(tenant.id);
        }
        next();
    }
    catch (error) {
...
exports.default = exports.tenantMiddleware;
